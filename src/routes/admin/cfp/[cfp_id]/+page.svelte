<script>
    ...
    import {customerList} from "$lib/CFP/store.js";
    ...
    customerList.subscribe(_=>{
        console.log(get(customerList))
    })
</script>
...
<table>
    ...
    <tbody>
    {#each $customerList as data, index}
        <tr>
            <td>{data.name}</td>
            <td>
                <select name="" id="" bind:value={selected[index]}>
                    {#each data.versions_info as version_info}
                        <option value={version_info.version_num}>
            ...
            <td>
                {#each data.versions_info as version_info}
                    {#if selected[index] && version_info.version_num===selected[index]}
                        {version_info.date}
                    {/if}
                {/each}
            </td>
            ...
            <td>
                <div class="status-container">
                    <div class="edit-view">
                        {#each data.versions_info as version_info}
                            {#if selected[index] && version_info.version_num===selected[index]}
                                <button on:click={()=>goto('/cfp/assets/'+version_info.version_id)}>
                                ...
                                <button on:click={()=>goto('/cfp/dashboard/'+version_info.version_id)}>
