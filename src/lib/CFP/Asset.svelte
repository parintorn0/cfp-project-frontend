    ...
    {#if asset.type === type}
        <div class="container">
                    ...
                    <div class="input-row">
                        <div class="input-field">
                            ...
                            <select name="" id="">
                                ...
                                {:else if type==='Private'}
                                    <option value="1" selected={asset.subtype==='RealEstate'}>อสังหาริมทรัพย์</option>
                                    <option value="2" selected={asset.subtype==='Depreciable'}>สินทรัพย์เสื่อมราคา</option>
                                    <option value="3" selected={asset.subtype==='Collection'}>ของสะสม</option>
                                    <option value="4" selected={asset.subtype==='Other'}>อื่นๆ</option>
                                {:else if type==='Investment'}
                                    <option value="1" selected={asset.subtype==='Bond'}>ตราสาร</option>
                                    <option value="2" selected={asset.subtype==='RealEstate'}>อสังหาริมทรัพย์</option>
                                    <option value="3" selected={asset.subtype==='Stock'}>หุ้น</option>
                                    <option value="4" selected={asset.subtype==='Other'}>อื่นๆ</option>
                                {:else if type==='Other'}
                                    <option value="1" selected={asset.subtype==='Copyright'}>ลิขสิทธิ์</option>
                                    <option value="2" selected={asset.subtype==='Patent'}>สิทธิบัตร</option>
                                    <option value="3" selected={asset.subtype==='Other'}>อื่นๆ</option>
                                {/if}
                            </select>
                        </div>
            ...
            <div class="btn-container">
                <div class="btn">
                    <Button style="display: flex; align-items: center; font-size: 16px;" color={{"r": 255, "g": 0, "b": 0, "a": 0.3}} onClick={()=>removeAsset(index)}>
